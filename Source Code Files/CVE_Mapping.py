...
def extract_commit_data(commit_df):
    ...
    commit_df['Commit_SHA'] = commit_df['sha/number']  # Use 'sha/number' for commit SHA
    return commit_df

# Function to map CVEs to commit messages using semantic similarity and include commit SHA/ID
    ...
    for cve_idx in range(len(cve_df)):
        ...
        for commit_idx, score in enumerate(scores):
            if score >= threshold:
                ...
                commit_sha = commit_row.get('Commit_SHA', '')  # Include the commit SHA/ID

                # Print each mapping being added
                print(f"Mapping: CVE_ID={cve_id}, Commit_SHA={commit_sha}, Commit_Message={commit_message}, Similarity_Score={score.item()}")

                mapped_data.append({
                    'CVE_ID': cve_id,  # From NIST data
                    'Commit_SHA': commit_sha,  # Include commit SHA/ID in the result
...
if __name__ == "__main__":
    ...
    cve_file_path = r'C:\Users\ADMIN\Desktop\bug-connector\Datasets\NIST_cve_data.csv'
    ...
    if not cve_df.empty and not commit_df.empty:
        ...
        if not mapped_df.empty:
            output_file_path = r'C:\Users\ADMIN\Desktop\bug-connector\Datasets\Mapped_CVE_file.csv'
